...
def biKmeans(dataSet, k, distMeans = distEclud):
    ...
    while (len(centList) < k):   #循环，直到达到k类
        ...
        for i in range(len(centList)):  #遍历每个聚类
            ...
            #输出两个误差值
            print("seeSplit, and not Split:",sseSplit, sseNotSplit)
            #划分第i类后总误差小于当前最小总误差
        ...
        # 打印本次执行2-均值聚类算法的类
        print( '本次执行k-均值聚类de类: ',bestCentToSplit)
        # 打印被划分的类的数据个数
        print('被划分的类的个数: ', len(bestClustAss))
        # 更新质心列表中的变化后的质心向量
...
if __name__ == '__main__':
    ...
    centList, clusterAssment = biKmeans(normAttribute, 3)
    print(centList)
