...
void EnQueue(LQueue *Q){   //入队函数 
	...
	while(flag){
		...
		if(Q->length == sizeof(char)){   //根据不同类型来定义不同的变量 
			char a;
			scanf("%c",&a);             //输入字符 
			enqueue_flag=EnLQueue(Q, &a);
			}
		else if (Q->length == sizeof(double)){
			double b;
			scanf("%lf",&b);          //输入浮点数 
			enqueue_flag=EnLQueue(Q, &b);
			}
		else if (Q->length == sizeof(int)){
			int c;
			scanf("%d",&c);         //输入整数 
			enqueue_flag=EnLQueue(Q, &c);
			}
		if(enqueue_flag == 1)
			printf("入队成功.");
		else 
...
void DeQueue(LQueue *Q){  //由于出队比较简单，就不需要分类型 
	int flag=1,dequeue_flag;
	while(flag){ 
		dequeue_flag=DeLQueue(Q);
		if (dequeue_flag==1)
			printf("\n出队成功.");
		else
			printf("\n出队失败。");
...
int main() {
	...
	switch(choice2){
		case '0': InitLQueue(&Q);break;  //初始化队列 
	...
	do{
		...
		switch(choice3){
			...
			case '4':{
				int length;
				length = LengthLQueue(&Q);  //返回队列长度 
				printf("\n该队列的长度是：%d",length);
				break;
			} 
			case '5':EnQueue(&Q);break;  //入队 
			case '6':DeQueue(&Q);break;   //出队 
			case '7':ClearLQueue(&Q);break;  //清空队列 
			case '8':{
